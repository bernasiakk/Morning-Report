...
# Functions
def extract_daily_word(task_instance):
    ...
    if result.status_code == 200:
        ...
        with open(tot_name, 'w') as outputfile:
            outputfile.write(str(soup)) # slight change vs. the JSON parsers
...
def transform_daily_word(task_instance): # get only the relevant data into the ./processed/ folder
    # Pull XCom variables from previous task
    current_datetime = task_instance.xcom_pull(task_ids='extract_daily_word', key='current_datetime')
    file_name = task_instance.xcom_pull(task_ids='extract_daily_word', key='file_name')
    # soup = task_instance.xcom_pull(task_ids='extract_daily_word', key='soup')
    ...
    word = (' '.join(child.find("title")))
    description = (' '.join(child.find("description")))
...
with models.DAG('daily_word',
    ...
    # Set DAGs
    extract_daily_word = PythonOperator(
        task_id='extract_daily_word',
        python_callable=extract_daily_word,
    )

    transform_daily_word = PythonOperator(
        task_id='transform_daily_word',
        python_callable=transform_daily_word,
    )

    # Pipe DAGs
    extract_daily_word >> transform_daily_word
