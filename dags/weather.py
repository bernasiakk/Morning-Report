...
# TODO swap all occurences of "weather" to "your_dag_name" in the file, as well as edit all lines marked as TODO

# Functions
def extract_weather(task_instance):
    # Specify API parameters & query the API 
    parameters = {'units': 'metric', # TODO
                'q': 'warszawa',
                'appid': API_KEY}

    result = requests.get(
        "http://api.openweathermap.org/data/2.5/weather?", parameters) # TODO
...
def transform_weather(task_instance): # get only the relevant data into the ./processed/ folder
    # Pull XCom variables from previous task
    current_datetime = task_instance.xcom_pull(task_ids='extract_weather', key='current_datetime')
    file_name = task_instance.xcom_pull(task_ids='extract_weather', key='file_name')

    # Download from GCP
    client = storage.Client()
    bucket = client.bucket(bucket_name)
    blob = bucket.blob(os.path.join(folder, file_name))
    # TODO edit the below 3 lines depending on your file extension (.json, .xml, etc.)
    content = blob.download_as_string().decode('utf-8').strip()
    file_object = StringIO(content)
    doc = json.load(file_object)    
    
    # TODO this whole section (extract what you're interested in, then match it to your table schema)
    ...
    sunrise = str(datetime.fromtimestamp(int(doc['sys']['sunrise']) + int(doc['timezone'])))
    sunset = str(datetime.fromtimestamp(int(doc['sys']['sunset']) + int(doc['timezone'])))
