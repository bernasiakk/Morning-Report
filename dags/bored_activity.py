...
# Functions
def extract_bored_activity(task_instance):
...
def transform_bored_activity(task_instance): # get only the relevant data into the ./processed/ folder
    # Pull XCom variables from previous task
    current_datetime = task_instance.xcom_pull(task_ids='extract_bored_activity', key='current_datetime')
    file_name = task_instance.xcom_pull(task_ids='extract_bored_activity', key='file_name')
...
with models.DAG('bored_activity',
    ...
    # Set DAGs
    extract_bored_activity = PythonOperator(
        task_id='extract_bored_activity',
        python_callable=extract_bored_activity,
    )

    transform_bored_activity = PythonOperator(
        task_id='transform_bored_activity',
        python_callable=transform_bored_activity,
    )

    # Pipe DAGs
    extract_bored_activity >> transform_bored_activity
